...
def main():
    ...
    print('Part 1:', len(colorDict) - 1)  # -1: it doesn't paint the last panel

    robot_code = intCodeRunner(input_array, 1)
    robot = robot_code.runAsync()
    ...
    while True:
        try:
            ...
            if direction % 2 == 0:
                location[1] += -1 + direction
                y_min = location[1] if location[1] < y_min else y_min
                y_max = location[1] if location[1] > y_max else y_max
            if direction % 2 != 0:
                location[0] += -1 if direction == 3 else 1
                x_min = location[0] if location[0] < x_min else x_min
                x_max = location[0] if location[0] > x_max else x_max
            if not (tuple(location) in colorDict):
                colorDict[tuple(location)] = 0
            robot_code.addInput(colorDict[tuple(location)])
        except StopIteration:
    ...
    for location, color in colorDict.items():
        if color == 1:
            x = location[0] + abs(x_min)
            y = location[1] + abs(y_min)
            picture[y][x] = 1
