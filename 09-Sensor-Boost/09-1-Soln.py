    ...
    def _add(self, index, param_modes):
        ...
        for (idx, mode) in enumerate(param_modes, 1):
            if mode == 0:
                ...
                pointers.append(self.instructionArray[index + idx] + self.rel_base)
        self.instructionArray[pointers[2]] = self.instructionArray[pointers[0]] + self.instructionArray[pointers[1]]
        index += 4
        return index
...
def main():
    ...
    test=intCodeRunner(input_array, 1)
    test.run()
